...
export default function Checkout() {
  ...
  const [method, setMethod] = useState<
    "upi" | "card" | "netbanking" | "otherupi" | "cod"
  >("upi");
  ...
  useEffect(() => {
    ...
    if (!selectedAddrId && readAddresses()[0])
      setSelectedAddrId(readAddresses()[0].id);
  }, []);
  ...
  const summary = useMemo(() => {
    ...
    return {
      ...
      taxIncluded,
      grandTotal,
      totalSavings: offerSavings + mrpSavings,
    };
  }, [items]);
  ...
  const placeOrder = async () => {
    ...
    const fallbackLocal = (message?: string) => {
      ...
      toast.success(
        message ||
          "Order placed (local demo). Connect Supabase/Razorpay for live payments.",
      );
    };
  ...
  return (
    <section className="container py-12">
        ...
        <div className="space-y-6">
            ...
            <div className="p-4 grid gap-3">
              ...
              <label className="flex items-center gap-3 rounded-lg border border-border p-3">
                <input
                  type="radio"
                  name="pm"
                  checked={method === "otherupi"}
                  onChange={() => setMethod("otherupi")}
                />{" "}
                Other UPI Apps
              </label>
              <label className="flex items-center gap-3 rounded-lg border border-border p-3">
          ...
          <div className="rounded-xl border border-border bg-background p-4 text-sm text-muted-foreground">
            <div className="flex items-center gap-2">
              <Gift className="h-4 w-4 text-primary" /> Use coupon SOLAR5 in
              cart for demo discount.
            </div>
            <div className="mt-2 flex items-center gap-2">
              <Package className="h-4 w-4 text-primary" /> Need GST invoice?
        ...
        <aside className="h-fit rounded-xl border border-border bg-muted/20 p-6">
          ...
          <div className="mt-4 space-y-3">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="text-foreground">{inr(summary.subtotal)}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">Shipping</span>
              <span className="text-foreground">
                {summary.shipping === 0 ? "Free" : inr(summary.shipping)}
              </span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">Tax included</span>
              <span className="text-foreground">
                {inr(summary.taxIncluded)}
              </span>
            </div>
          </div>
            ...
            {items.map((item) => (
              ...
              >
                ...
                <div className="text-sm font-semibold text-foreground">
                  {inr(
                    calculateOfferPricing(item.price, item.offer).finalPrice *
                      item.quantity,
                  )}
                </div>
              </div>
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            <Truck className="mr-1 inline h-4 w-4 text-primary" /> Delivery
      ...
      <Dialog
        open={success.open}
        onOpenChange={(open) => setSuccess((c) => ({ ...c, open }))}
      >
        <DialogContent>
          <DialogHeader>
